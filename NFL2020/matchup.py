...
def get_opponent_stats(opponent, venue):
    ...
    opp_stats = pd.read_csv(os.path.join(teamsheetpath, opponent + '.csv'), index_col = 0)
...
def get_expected_scores(team_1_stats, team_2_stats, team_1_df, team_2_df):
    ...
    for stat in expected_scores:
        expected_scores[stat] = max(expected_scores[stat], 0.01)
...
def matchup(team_1, team_2, venue = None):
    ...
    ts = time.time()
    team_1_season = pd.read_csv(os.path.join(teamsheetpath, team_1 + '.csv'), index_col = 0)
    team_2_season = pd.read_csv(os.path.join(teamsheetpath, team_2 + '.csv'), index_col = 0)
    ...
    games = pd.DataFrame({team_1: team_1_scores, team_2: team_2_scores})
