...
regression_file = os.path.join(base_path, 'RegressionResults_Alt.txt')
f = open(regression_file, 'w')
f.write(str(res.summary()))
f.close()

#Obtain color gradient for plotting
color_gradient = color_interpolate('#ff0000', '#0000ff', 17)
...
plt.plot([xmin, xmax], [ymin, ymax], 'b--', label = 'Week 15+')
...
title_lines = ['Merrer Predictifier Validation: 2018 NFL Season',
               'Actual Scores vs. Expected Scores (Average of 5,000,000 Simulations)',
               'Week 15+: Actual = {0}\u00d7Expected + {1} + \u03b5, r\u00b2 = {2}'.format(round(res.params[0], 2),
...
#Write plot to file
scatterplot_file = os.path.join(base_path, 'ScoreScatterplot_Alt.png')
...
plt.xlim(0, 1)
plt.title('Week 15+ Score Distribution Validation')
plt.ylabel('% of Scores within\nForecast Quartiles')

#Write plot to file
barplot_file = os.path.join(base_path, 'QuartileBarPlot_Alt.png')
